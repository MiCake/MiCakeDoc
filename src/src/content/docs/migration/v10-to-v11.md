---
title: v10 → v11 迁移指南
description: 从 v10 升级到 v11 的完整迁移指南：所有破坏性变更、迁移步骤与新 API 速查
---

本指南帮助您将 MiCake 应用从 v10 升级到 v11。v11 将 **Unit of Work 确立为唯一持久化所有者**，并改为**自动安装** EF Core 拦截器，涉及仓储 API、UoW API、ASP.NET Core 边界与 EF Core 集成四个层面的破坏性变更。
...
原来使用 `AddAndReturnAsync` 的地方，**建议迁移到新加入接口的
...
> 用于只读 filter/middleware）。无 UoW 的**写入**整体按 Permissive 策略放行，
> 不再需要、也不应依赖此选项放行写入。
...
以前会拒绝无 UoW 的直接 `DbContext` 写入；现在按原生 EF Core 语义**放行**（隐式事务，
...
| `IRepository<TAggregateRoot, TKey>.AddAndGetIdAsync(...)` | `AddAndReturnAsync` 的**推荐迁移目标**：添加 + FlushAsync 填充生成键并返回 `TKey`（需环境可写 UoW） |
...
- [仓储](/domain-driven/repository/) — v11 仓储 API 详解
- [工作单元](/domain-driven/unit-of-work/) — v11 UoW API 详解
- [领域事件](/domain-driven/domain-event/) — 事件派发时机说明
